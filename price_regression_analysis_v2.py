...
def prepare_features_and_target(df):
    ...
    # Target variable - now using Net price / part
...
def correlation_analysis(X, y):
    ...
    print("\n" + "="*60)
    print("CORRELATION ANALYSIS WITH NET PRICE/PART")
    print("="*60)
    ...
    print(f"\nTop 15 Features by Correlation with Net Price/Part:")
    print("-" * 65)
...
def generate_insights(results, feature_importance, corr_df, best_model_name):
    ...
    print(f"   - Explains {best_r2:.1%} of net price variation")
    print(f"   - Average prediction error: {results[best_model_name]['test_mape']:.1f}%")
    if best_r2 < 0.5:
    ...
    if feature_importance is not None:
        print(f"\n2. TOP NET PRICE DRIVERS:")
        top_5 = feature_importance.head(5)
        for i, (_, row) in enumerate(top_5.iterrows(), 1):
            direction = "increases" if row['Coefficient'] > 0 else "decreases"
            print(f"   {i}. {row['Feature']} - {direction} net price")
...
def main():
    ...
    print("🔍 STARTING NET PRICE REGRESSION ANALYSIS FOR PA2200 PARTS")
    print("="*80)
    ...
    try:
        ...
        if feature_importance is not None:
            feature_importance.to_csv('net_price_feature_importance_analysis.csv', index=False)
            print(f"\n📊 Detailed feature importance saved to: net_price_feature_importance_analysis.csv")
        
        corr_df.to_csv('net_price_correlation_analysis.csv', index=False)
        print(f"📊 Correlation analysis saved to: net_price_correlation_analysis.csv")
