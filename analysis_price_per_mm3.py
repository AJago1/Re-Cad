...
def main():
    print("📊 COMPREHENSIVE ANALYSIS - PRICE PER MM³")
    print("=" * 55)
    ...
    # Use Net price/mm3 as target (standard 3D printing metric)
    price_features = ['Net price / part', 'net price * Quantity']  # Exclude absolute prices
    ...
    print(f"🎯 Target: {target} (Standard 3D printing metric)")
    ...
    try:
        ...
        # Remove the target itself and sort
        ...
        print(f"\n📊 Correlation analysis skipped due to error: {e}")
    ...
    if 'Colour' in df_clean.columns:
        ...
        for color, count in color_counts.head(8).items():
            pct = count / len(df_clean) * 100
            avg_price_per_mm3 = df_clean[df_clean['Colour'] == color][target].mean()
            print(f"   • {color}: {count:,} parts ({pct:.1f}%), Avg €{avg_price_per_mm3:.6f}/mm³")
    ...
    if 'Machine' in df_clean.columns:
        ...
        if len(machine_comparison) > 1:
            ...
            print(f"\n   📊 Price Ratio: {ratio:.1f}x difference between most/least expensive machines")
    
    print(f"\n💡 KEY INSIGHTS (Price per mm³):")
    print(f"   • Using standard 3D printing metric (€/mm³) normalizes for part size")
    print(f"   • Machine type shows clear pricing differences per volume")
    print(f"   • Color variations affect cost efficiency")
    print(f"   • {combined_outliers.sum()} outliers with unusual €/mm³ ratios")
    print(f"   • Random Forest best captures pricing complexity")
    
    return df_clean, results, available_features

if __name__ == "__main__":
    df, results, features = main()
    print("\n✅ Price per mm³ analysis complete!") 