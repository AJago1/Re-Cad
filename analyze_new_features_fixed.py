...
# New interaction features (note the trailing space in 'Volume * Quantity ')
...
df['complexity_score'] = df['Surface area*Quantity'] / df['Volume * Quantity ']  # Surface/volume accounting for quantity
...
print("\n=== PRICING INSIGHTS ===")
# Price per mm3 analysis
df['price_per_mm3'] = df['price'] / df['Volume']
df['price_per_mm3_bulk'] = df['price'] / df['Volume * Quantity ']

print(f"4. PRICE SCALING:")
print(f"   Average price per mm³ (individual): {df['price_per_mm3'].mean():.6f}")
print(f"   Average price per mm³ (bulk-adjusted): {df['price_per_mm3_bulk'].mean():.6f}")

# Quantity effect analysis
qty_effect = df.groupby('qty_bin').agg({
    'price_per_mm3': 'mean',
    'price_per_mm3_bulk': 'mean'
}).round(6)
print(f"\nPrice per mm³ by quantity bracket:")
print(qty_effect)
...
if surface_qty_corr > surface_corr:
    print(f"   ✓ Surface*Quantity improves prediction by {((surface_qty_corr/surface_corr-1)*100):+.1f}%")
else:
    print(f"   ✗ Surface*Quantity doesn't improve over Surface alone")

print("\n3. NEXT STEPS TO TRY:")
print("   - log(Volume*Quantity) for non-linear scaling effects")  
print("   - Machine dummy variables (Formiga vs P396 price differently)")
print("   - Quantity bins as categorical features")
print("   - Project size interaction: (Volume*Quantity) / Total_Parts_in_Project")
print("   - Complexity penalty: Surface*Quantity / (Volume*Quantity)²") 