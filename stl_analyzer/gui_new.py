...
try:
    ...
    from .gui.threads import ScanThread, ProcessThread, StepConversionThread
except ImportError:
    # When run directly
    from stl_utils import extract_features, extract_features_with_shrinkwrap, is_valid_entry, find_similar, try_load_stl
    from database import STLDatabase
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setup_analyzer_page(self):
        ...
        main_layout = QtWidgets.QHBoxLayout(self.analyzer_widget)
    ...
    def setup_scan_controls(self):
        ...
        self.shrinkwrap_output_mode.setToolTip(
            ...
            "‘«Û Dedicated Folder: In a 'shrinkwrap' subfolder (recommended)\n"
            "‘«Û Same Directory: Next to original files (clutters directory)\n"
            "‘«Û Custom Directory: In a user-specified location"
        )
        ...
        # Status info
        self.shrinkwrap_status_label = QtWidgets.QLabel("‘ù˘ Data calculation enabled - File generation disabled (clean scan)")
    ...
    def load_comparison_part(self):
        ...
        try:
            ...
            self.show_status_message(
                f"Loaded: {filename} (Vol: {volume:.2f}mmT-, BBox: {bbox_vol:.2f}mmT-, SA: {surface_area:.2f}mmT-)"
            )
    ...
    def setup_converter_page(self):
        ...
        converter_layout = QtWidgets.QVBoxLayout(self.converter_widget)
        ...
        # Add to main layout
        converter_layout.addWidget(converter_group)
    ...
    def setup_price_calculator_page(self):
        ...
        # Create price calculator widget
        self.price_calculator_widget = QtWidgets.QWidget()
        price_calculator_layout = QtWidgets.QVBoxLayout(self.price_calculator_widget)
        ...
        self.save_project_button.clicked.connect(self.save_price_to_project)
        self.save_project_button.setEnabled(False)  # Disabled until calculations are done
        right_buttons_layout.addWidget(self.save_project_button)
        ...
        # Create main content splitter (horizontal)
        ...
        self.total_project_price_label = QtWidgets.QLabel("‘È»0.00")
        ...
        # Top middle - Selected Part Details (ENHANCED)
        ...
        # Part detail fields - EXPANDED to show all important features
        ...
        self.part_convex_hull_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Convex Hull:", self.part_convex_hull_label)
        
        self.part_bb_volume_label = QtWidgets.QLabel("")
        part_details_layout.addRow("BB Volume:", self.part_bb_volume_label)
        
        # Add shrinkwrap info
        self.part_shrinkwrap_volume_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Shrinkwrap Vol:", self.part_shrinkwrap_volume_label)
        
        self.part_shrinkwrap_ratio_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Shrinkwrap Ratio:", self.part_shrinkwrap_ratio_label)
        
        # Add dimensional information
        self.part_max_dim_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Max Dimension:", self.part_max_dim_label)
        
        self.part_min_dim_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Min Dimension:", self.part_min_dim_label)
        
        self.part_dimension_ratio_label = QtWidgets.QLabel("")
        part_details_layout.addRow("D Ratio:", self.part_dimension_ratio_label)
        ...
        self.part_waste_volume_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Waste Volume:", self.part_waste_volume_label)
        
        self.part_waste_ratio_label = QtWidgets.QLabel("")
        part_details_layout.addRow("Waste Ratio:", self.part_waste_ratio_label)
        
        # Quantity selector
        self.part_quantity_spinner = QtWidgets.QSpinBox()
        self.part_quantity_spinner.setMinimum(1)
        self.part_quantity_spinner.setMaximum(1000)
        ...
        pricing_selector_layout = QtWidgets.QHBoxLayout()
        pricing_selector_layout.addWidget(QtWidgets.QLabel("Pricing Preset:"))
        
        self.pricing_preset_combo = QtWidgets.QComboBox()
        self.pricing_preset_combo.currentIndexChanged.connect(self.on_pricing_preset_changed)
        pricing_selector_layout.addWidget(self.pricing_preset_combo, 1)
        
        # Quick Add Preset button
        add_preset_button = QtWidgets.QPushButton("Add New")
        add_preset_button.clicked.connect(self.add_new_pricing_preset)
        pricing_selector_layout.addWidget(add_preset_button)
        
        middle_layout.addLayout(pricing_selector_layout)
        
        # Enhanced Linear Model Options Group
        linear_model_group = QtWidgets.QGroupBox("AI Pricing Model Options")
        linear_model_layout = QtWidgets.QVBoxLayout(linear_model_group)
        
        # Enable AI Model checkbox
        linear_model_checkbox_layout = QtWidgets.QHBoxLayout()
        self.use_ai_model_checkbox = QtWidgets.QCheckBox("Use AI Model")
        self.use_ai_model_checkbox.setToolTip("Use the advanced AI model trained on historical pricing data")
        self.use_ai_model_checkbox.setChecked(True)  # Default to enabled
        self.use_ai_model_checkbox.stateChanged.connect(self.on_ai_model_toggled)
        linear_model_checkbox_layout.addWidget(self.use_ai_model_checkbox)
        linear_model_layout.addLayout(linear_model_checkbox_layout)
        
        # Model Selection Section
        model_selection_layout = QtWidgets.QHBoxLayout()
        model_selection_layout.addWidget(QtWidgets.QLabel("Pricing Model:"))
        
        self.pricing_model_combo = QtWidgets.QComboBox()
        self.pricing_model_combo.addItem("Default (Built-in)")
        self.pricing_model_combo.currentTextChanged.connect(self.on_pricing_model_changed)
        model_selection_layout.addWidget(self.pricing_model_combo, 1)
        
        # Load Model button
        self.load_model_button = QtWidgets.QPushButton("Load Model")
        self.load_model_button.clicked.connect(self.load_pricing_model)
        model_selection_layout.addWidget(self.load_model_button)
        
        # Refresh Models button
        self.refresh_models_button = QtWidgets.QPushButton("Refresh")
        self.refresh_models_button.clicked.connect(self.refresh_pricing_models)
        model_selection_layout.addWidget(self.refresh_models_button)
        
        linear_model_layout.addLayout(model_selection_layout)
        
        # Model Info display
        self.model_info_label = QtWidgets.QLabel("AI Model | RT- = 94.3% | MAE = ‘È»19.45")
        self.model_info_label.setStyleSheet("font-size: 10px; color: #666; font-style: italic;")
        linear_model_layout.addWidget(self.model_info_label)
        
        # Model accuracy display
        accuracy_layout = QtWidgets.QHBoxLayout()
        accuracy_layout.addWidget(QtWidgets.QLabel("Model Accuracy:"))
        self.model_accuracy_label = QtWidgets.QLabel("Training...")
        self.model_accuracy_label.setStyleSheet("color: #0066cc; font-weight: bold;")
        accuracy_layout.addWidget(self.model_accuracy_label)
        linear_model_layout.addLayout(accuracy_layout)
        
        # Add spacer and note
        note_label = QtWidgets.QLabel("‘‰¶¥™∆ Pure AI model pricing - no manufacturing calculations")
        note_label.setStyleSheet("font-size: 9px; color: #888; font-style: italic;")
        linear_model_layout.addWidget(note_label)
        

        
        # Add to middle layout
        middle_layout.addWidget(linear_model_group)
        
        # Bottom middle - Cost Breakdown
        cost_breakdown_group = QtWidgets.QGroupBox("Cost Breakdown")
        cost_breakdown_group.setObjectName("cost_breakdown_group")
        cost_breakdown_layout = QtWidgets.QFormLayout(cost_breakdown_group)
        
        # Cost breakdown fields
        self.material_cost_label = QtWidgets.QLabel("‘È»0.00")
        ...
        # Complexity factor indicator
        self.complexity_factor_label = QtWidgets.QLabel("1.00+å")
        complexity_tooltip = (
            "Complexity multiplier applied to printing time.\n"
            "1.00+å = Simple shape\n"
            "Higher values = More complex shapes that take longer to print"
        )
        self.complexity_factor_label.setToolTip(complexity_tooltip)
        cost_breakdown_layout.addRow("Complexity Factor:", self.complexity_factor_label)
        
        self.machine_cost_label = QtWidgets.QLabel("‘È»0.00")
        cost_breakdown_layout.addRow("Machine Cost:", self.machine_cost_label)
        
        self.energy_cost_label = QtWidgets.QLabel("‘È»0.00")
        cost_breakdown_layout.addRow("Energy Cost:", self.energy_cost_label)
        
        self.labor_cost_label = QtWidgets.QLabel("‘È»0.00")
        cost_breakdown_layout.addRow("Labor Cost:", self.labor_cost_label)
        
        self.maintenance_cost_label = QtWidgets.QLabel("‘È»0.00")
        cost_breakdown_layout.addRow("Maintenance Buffer:", self.maintenance_cost_label)
        
        # AI model price
        self.ai_model_price_label = QtWidgets.QLabel("‘È»0.00")
        self.ai_model_price_row = cost_breakdown_layout.addRow("AI Model Price:", self.ai_model_price_label)
        
        # Custom adjustments
        self.custom_adjustment_input = QtWidgets.QDoubleSpinBox()
        self.custom_adjustment_input.setRange(-1000, 1000)
        self.custom_adjustment_input.setValue(0)
        self.custom_adjustment_input.setPrefix("‘È»")
        self.custom_adjustment_input.valueChanged.connect(self.on_pricing_input_changed)
        cost_breakdown_layout.addRow("Custom Adjustment:", self.custom_adjustment_input)
        
        # Separator line
        separator = QtWidgets.QFrame()
        separator.setFrameShape(QtWidgets.QFrame.HLine)
        separator.setFrameShadow(QtWidgets.QFrame.Sunken)
        cost_breakdown_layout.addRow("", separator)
        
        # Base total
        self.base_total_label = QtWidgets.QLabel("‘È»0.00")
        self.base_total_label.setStyleSheet("font-weight: bold;")
        ...
        self.margin_input.setValue(15)
        self.margin_input.setSingleStep(5)
        self.margin_input.setSuffix("%")
        self.margin_input.valueChanged.connect(self.on_pricing_input_changed)
        cost_breakdown_layout.addRow("Margin (%):", self.margin_input)
        
        # Final price
        self.final_price_label = QtWidgets.QLabel("‘È»0.00")
        self.final_price_label.setStyleSheet("font-weight: bold; font-size: 14px;")
        ...
        self.save_to_project_button = QtWidgets.QPushButton("Add to Project")
        self.save_to_project_button.clicked.connect(self.save_price_to_project)
        self.save_to_project_button.setEnabled(False)  # Disabled until a valid calculation is made
        bottom_buttons_layout.addWidget(self.save_to_project_button)
        ...
        # Right panel - 3D Viewer and Feature Contribution
        right_panel = QtWidgets.QWidget()
        right_layout = QtWidgets.QVBoxLayout(right_panel)
        ...
        self.price_calc_bb_checkbox = QtWidgets.QCheckBox("Bounding Box")
        self.price_calc_bb_checkbox.setChecked(True)
        self.price_calc_bb_checkbox.stateChanged.connect(
            lambda state: self.price_calc_viewer.set_bounding_box_visible(state == Qt.Checked)
        )
        view_controls_layout.addWidget(self.price_calc_bb_checkbox)
        
        # Edges checkbox
        self.price_calc_edges_checkbox = QtWidgets.QCheckBox("Edges")
        self.price_calc_edges_checkbox.setChecked(True)
        self.price_calc_edges_checkbox.stateChanged.connect(
            lambda state: self.price_calc_viewer.set_edges_visible(state == Qt.Checked)
        )
        view_controls_layout.addWidget(self.price_calc_edges_checkbox)
        
        # Convex Hull checkbox
        self.price_calc_convex_hull_checkbox = QtWidgets.QCheckBox("Convex Hull")
        self.price_calc_convex_hull_checkbox.setChecked(False)
        ...
        self.price_calc_convex_hull_checkbox.setToolTip(
            "Show the convex hull of the model - the smallest convex shape that contains the model.\n"
            "The ratio between model volume and convex hull volume affects pricing through the complexity factor."
        )
        view_controls_layout.addWidget(self.price_calc_convex_hull_checkbox)
        
        # Shrinkwrap checkbox
        self.price_calc_shrinkwrap_checkbox = QtWidgets.QCheckBox("Shrinkwrap")
        self.price_calc_shrinkwrap_checkbox.setChecked(False)
        ...
        self.price_calc_shrinkwrap_checkbox.setToolTip(
            "Show the shrinkwrap of the model - 4mm offset from the original surface.\n"
            "Used for material usage calculations and complexity analysis."
        )
        view_controls_layout.addWidget(self.price_calc_shrinkwrap_checkbox)
        ...
        viewer_inner_layout.addWidget(reset_view_button)
        
        viewer_layout.addWidget(viewer_group)
        right_splitter.addWidget(viewer_panel)
        
        # Bottom right - Feature Contribution Chart
        chart_panel = QtWidgets.QWidget()
        chart_layout = QtWidgets.QVBoxLayout(chart_panel)
        
        # Feature contribution group
        contribution_group = QtWidgets.QGroupBox("AI Model Price Factors")
        contribution_layout = QtWidgets.QVBoxLayout(contribution_group)
        
        # Create a table for feature contributions
        self.feature_contribution_table = QtWidgets.QTableWidget()
        self.feature_contribution_table.setColumnCount(3)
        self.feature_contribution_table.setHorizontalHeaderLabels(["Feature", "Value", "Price Impact"])
        self.feature_contribution_table.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        self.feature_contribution_table.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)  # Read-only
        contribution_layout.addWidget(self.feature_contribution_table)
        
        # Total contribution
        total_contribution_layout = QtWidgets.QHBoxLayout()
        total_contribution_layout.addWidget(QtWidgets.QLabel("AI Model Price:"))
        self.total_ai_price_label = QtWidgets.QLabel("‘È»0.00")
        self.total_ai_price_label.setStyleSheet("font-weight: bold; color: #0066cc;")
        total_contribution_layout.addWidget(self.total_ai_price_label)
        contribution_layout.addLayout(total_contribution_layout)
        
        chart_layout.addWidget(contribution_group)
        right_splitter.addWidget(chart_panel)
        
        # Set initial splitter sizes (60% 3D viewer, 40% feature contribution)
        right_splitter.setSizes([600, 400])
        
        # Add right panel to the main splitter
        self.price_calc_splitter.addWidget(right_panel)
        
        # Set initial sizes (25% left, 35% middle, 40% right)
        ...
        # Dictionary to store part data
        self.price_calc_parts = {}
        
        # Dictionary to store project parts
        self.project_parts = {}
        self.project_total_price = 0.0
        
        # Dictionary to store pricing configurations
        self.pricing_presets = {}
        
        # Initialize model name and attributes
        ...
        default_preset = {
            ...
            "monitoring_time": 0.1,  # +å build time
    ...
    def setup_project_pricing_page(self):
        ...
        self.pricing_analysis_button = QtWidgets.QPushButton("≠ËÙ’ Pricing Analysis Report")
        ...
        self.project_parts_table.setHorizontalHeaderLabels([
            "Part Name", "Quantity", "Volume (mmT-)", "Max Dim (mm)", "Unit Price", "Total Price", "Remove"
        ])
    ...
    def update_details_view(self, data):
        ...
        try:
            ...
            if isinstance(volume, (int, float)) and volume > 0:
                self.detail_volume.setText(f"{volume:.2f} mmT-")
            else:
            ...
            if isinstance(surface_area, (int, float)) and surface_area > 0:
                self.detail_surface.setText(f"{surface_area:.2f} mmT-")
            else:
            ...
            if isinstance(bbox_volume, (int, float)) and bbox_volume > 0:
                self.detail_bb_volume.setText(f"{bbox_volume:.2f} mmT-")
            else:
            ...
            if isinstance(ch_volume, (int, float)) and ch_volume > 0:
                self.detail_convex_hull_volume.setText(f"{ch_volume:.2f} mmT-")
            else:
            ...
            if isinstance(sw_volume, (int, float)) and sw_volume > 0:
                self.detail_shrinkwrap_volume.setText(f"{sw_volume:.2f} mmT-")
            else:
            ...
            if isinstance(waste_volume, (int, float)) and waste_volume > 0:
                self.detail_waste.setText(f"{waste_volume:.2f} mmT-")
            else:
    ...
    def on_file_generation_toggled(self, checked):
        ...
        if checked:
            if self.calculate_shrinkwrap_checkbox.isChecked():
                self.shrinkwrap_status_label.setText("‘ù˘ Data calculation + File generation enabled")
            else:
                self.shrinkwrap_status_label.setText("‘‹·¥™∆ File generation enabled but data calculation disabled")
        else:
            if self.calculate_shrinkwrap_checkbox.isChecked():
                self.shrinkwrap_status_label.setText("‘ù˘ Data calculation enabled - File generation disabled (clean scan)")
            else:
                self.shrinkwrap_status_label.setText("‘£Ó Both data calculation and file generation disabled")
    ...
    def start_scan(self):
        ...
        if current_entries > 0:
            ...
            reply = QtWidgets.QMessageBox.question(
                ...
                f"‘«Û Yes: Add new files to existing database\n"
                f"‘«Û No: Save current database and start new one\n"
                f"‘«Û Cancel: Cancel scanning",
        ...
        try:
            ...
            if not self.stl_db.get_all_entries().empty:
                reply = QtWidgets.QMessageBox.question(
                    ...
                    f"Do you want to:\n"
                    f"‘«Û Yes: Merge with current database\n"
                    f"‘«Û No: Replace current database\n"
                    f"‘«Û Cancel: Cancel loading",
                    QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No | QtWidgets.QMessageBox.Cancel,
                    QtWidgets.QMessageBox.Yes
                )
    ...
    def on_ai_model_toggled(self, state):
        ...
        if hasattr(self, 'enhanced_pricing_available'):
            if use_ai and not self.enhanced_pricing_available:
                self.show_status_message("‘‹·¥™∆ AI model not available, using standard pricing", 3000)
                self.use_ai_model_checkbox.setChecked(False)
            elif use_ai:
                self.show_status_message("‘ù˘ AI model enabled", 2000)
            else:
                self.show_status_message("≠ËÙ’ Standard pricing enabled", 2000)
        self.recalculate_price()
    ...
    def load_pricing_model(self):
        ...
        if current_text and current_text in self.available_models:
            ...
            self.show_status_message(f"‘ù˘ Loaded model: {current_text}", 2000)
        else:
            self.show_status_message("‘‹·¥™∆ No model selected", 2000)
    
    def refresh_pricing_models(self):
        """Refresh the available pricing models"""
        self.scan_available_models()
        self.show_status_message("≠Ëˆ‰ Models refreshed", 2000)
    ...
    def calculate_part_price(self, features, pricing_config, quantity=1):
        ...
        if hasattr(self, 'comprehensive_pricing_enabled') and self.comprehensive_pricing_enabled:
            ...
            if comprehensive_price and comprehensive_price > 0:
                print(f"≠Ëˆ» Comprehensive model prediction: ‘È»{comprehensive_price:.2f}")
        ...
        if self.use_ai_model_checkbox.isChecked() and hasattr(self, 'ai_model') and self.ai_model is not None:
            ...
            print(f"≠Ë•æ Legacy AI model prediction: ‘È»{ai_price:.2f}")
        ...
        if use_ai:
            # Pure AI model only
            base_total = ai_price
            print(f"≠Ë•æ Using AI prediction: ‘È»{ai_price:.2f}")
        else:
            # No AI model - set base price to 0
            base_total = 0.0
            print(f"‘£Ó AI model disabled or failed")
        ...
        try:
            ...
            if self.use_ai_model_checkbox.isChecked() and hasattr(self, 'ai_model') and self.ai_model is not None:
                ...
                if ai_price and ai_price > 0:
                    ...
                    self.material_cost_label.setText(f"Volume: {vol:.0f} mmT-")
                    self.estimated_time_label.setText(f"SW Vol: {sw_vol:.0f} mmT-")
                    self.machine_cost_label.setText(f"CH Vol: {ch_vol:.0f} mmT-")
                    self.energy_cost_label.setText(f"Surface: {surf_area:.0f} mmT-")
                    ...
                    self.ai_model_price_label.setText(f"‘È»{ai_price:.2f}")
                    self.total_ai_price_label.setText(f"‘È»{ai_price:.2f}")
                    ...
                    # Display prices
                    self.base_total_label.setText(f"‘È»{base_price:.2f}")
                    self.final_price_label.setText(f"‘È»{final_price:.2f}")
                    ...
                    self.machine_cost_label.setText("‘È»0.00")
                    self.energy_cost_label.setText("‘È»0.00")
                    self.labor_cost_label.setText("‘È»0.00")
                ...
                self.energy_cost_label.setText("‘È»0.00")
                self.labor_cost_label.setText("‘È»0.00")
                self.maintenance_cost_label.setText("‘È»0.00")
                self.complexity_factor_label.setText("N/A")
                self.ai_model_price_label.setText("‘È»0.00")
                self.total_ai_price_label.setText("‘È»0.00")
                self.base_total_label.setText("‘È»0.00")
                self.final_price_label.setText("‘È»0.00")
                self.save_to_project_button.setEnabled(False)
        ...
        try:
            ...
            for feature_name, coefficient in self.ai_feature_coefficients.items():
                if feature_name in feature_mapping:
                    ...
                    # Price impact
                    impact_text = f"‘È»{contribution:.2f}"
                    if contribution > 0:
            ...
            self.total_ai_price_label.setText(f"‘È»{total_contribution:.2f}")
    ...
    def update_part_details_display(self, features):
        ...
        try:
            ...
            details.append(f"Volume: {features.get('volume', 0):,.0f} mmT-")
            details.append(f"Surface Area: {features.get('surface_area', 0):,.0f} mmT-")
            details.append(f"Bounding Box: {features.get('x', 0):.1f} +å {features.get('y', 0):.1f} +å {features.get('z', 0):.1f} mm")
            
            if 'shrinkwrap_volume' in features:
                details.append(f"Shrinkwrap Volume: {features.get('shrinkwrap_volume', 0):,.0f} mmT-")
            if 'convex_hull_volume' in features:
                details.append(f"Convex Hull Volume: {features.get('convex_hull_volume', 0):,.0f} mmT-")
    ...
    def update_price_calculator_ui(self):
        ...
        if not current_item:
            ...
            self.material_cost_label.setText("‘È»0.00")
            self.estimated_time_label.setText("0.0 h")
            self.complexity_factor_label.setText("1.00+å")
            self.machine_cost_label.setText("‘È»0.00")
            self.energy_cost_label.setText("‘È»0.00")
            self.labor_cost_label.setText("‘È»0.00")
            self.maintenance_cost_label.setText("‘È»0.00")
    ...
    def save_price_to_project(self):
        ...
        # Show confirmation
        self.show_status_message(f"‘ù˘ Added '{part_name}' (‘È»{final_price:.2f} +å {quantity}) to project")
        print(f"‘ù˘ Added to project: {part_name} - ‘È»{final_price:.2f} +å {quantity} = ‘È»{final_price * quantity:.2f}")
    ...
    def initialize_ai_pricing_model(self):
        ...
        def initialize_random_forest():
            ...
            try:
                ...
                if all(os.path.exists(f) for f in project_aware_files):
                    print("≠ËÓ- Loading Project-Aware Random Forest model...")
                    ...
                    print("≠Ëƒª Project-Aware Random Forest model loaded successfully!")
                    print(f"   Features: {len(self.feature_names)} (including project context)")
                    print(f"   Test RT-: {self.model_r2_score:.4f}")
                    print(f"   Test MAE: ‘È»{self.model_mae:.2f}")
                    
                    self.show_status_message(f"‘ù˘ Project-Aware Random Forest loaded (RT- = {self.model_r2_score:.3f}, MAE = ‘È»{self.model_mae:.2f})")
                    return True
                ...
                if all(os.path.exists(f) for f in standard_files):
                    print("≠ËÓ- Loading Standard Random Forest model...")
                    ...
                    print("≠Ëƒª Standard Random Forest model loaded successfully!")
                    self.show_status_message(f"‘ù˘ Random Forest model loaded (RT- = {self.model_r2_score:.3f}, MAE = ‘È»{self.model_mae:.2f})")
                    return True
                
                print("‘‹·¥™∆ No Random Forest model files found")
                return False
        ...
        print("‘‹·¥™∆ No pre-trained Random Forest model found, using Lasso model")
        self.show_status_message("‘‹·¥™∆ Using Lasso model (Random Forest not trained)")
        ...
        try:
            ...
            print(f"‘£Ó Failed to initialize fallback Lasso model: {e}")
            self.show_status_message("‘£Ó Failed to initialize pricing model")
    ...
    def predict_price_with_ai_model(self, features_or_part):
        ...
        try:
            if not hasattr(self, 'pricing_model') or self.pricing_model is None:
                print("‘‹·¥™∆ No pricing model available")
                return None
            ...
            if hasattr(self, 'feature_names') and self.feature_names:
                ...
                print("‘‹·¥™∆ No feature names available for model")
                return None
                
        except Exception as e:
            print(f"‘£Ó AI model prediction failed: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def update_model_info_display(self):
        ...
        if hasattr(self, 'model_status_label'):
            if hasattr(self, 'enhanced_pricing_available') and self.enhanced_pricing_available:
                self.model_status_label.setText("Model Status: ‘ù˘ Random Forest Model Ready")
                self.model_accuracy_label.setText(f"RT- Score: {self.model_r2_score:.3f}, MAE: ‘È»{self.model_mae:.2f}")
                model_info = []
                model_info.append("<b>≠Ë‹« Random Forest Model:</b>")
                model_info.append(f"‘«Û Model Type: Random Forest Regressor")
                model_info.append(f"‘«Û Accuracy (RT-): {self.model_r2_score:.3f}")
                model_info.append(f"‘«Û Mean Absolute Error: ‘È»{self.model_mae:.2f}")
                model_info.append(f"‘«Û Features: {len(self.feature_names)}")
                if hasattr(self, 'top_features'):
                    model_info.append("<br><b>≠Ëˆπ Top 5 Most Important Features:</b>")
            ...
            if self.pricing_model is not None:
                self.model_status_label.setText("Model Status: ‘ù˘ Standard Lasso Ready")
                mae_text = f", MAE: ‘È»{self.model_mae:.2f}" if hasattr(self, 'model_mae') else ""
                self.model_accuracy_label.setText(f"RT- Score: {self.model_r2_score:.3f}{mae_text}")
                model_info = []
                model_info.append("<b>≠Ëƒª Standard Lasso Regression Model:</b>")
                model_info.append(f"‘«Û Model Type: Lasso (L1 Regularization)")
                model_info.append(f"‘«Û Accuracy (RT-): {self.model_r2_score:.3f}")
                model_info.append(f"‘«Û Mean Absolute Error: ‘È»{getattr(self, 'model_mae', 0):.2f}")
                model_info.append(f"‘«Û Selected Features: {len(self.feature_names)}/21 optimal")
                if hasattr(self, 'top_features'):
                    model_info.append("<br><b>≠Ëˆπ Top 5 Most Important Features:</b>")
                    for i, feature in enumerate(self.top_features[:5], 1):
    ...
    def update_project_part_details(self, part):
        ...
        self.project_detail_volume.setText(f"{volume:,.0f} mmT-")
        self.project_detail_surface.setText(f"{surface_area:,.0f} mmT-")
        self.project_detail_bb_volume.setText(f"{bb_volume:,.0f} mmT-")
        
        # Waste calculation
        waste = bb_volume - volume if bb_volume > 0 and volume > 0 else 0
        waste_percent = (waste / bb_volume * 100) if bb_volume > 0 else 0
        self.project_detail_waste.setText(f"{waste:,.0f} mmT- ({waste_percent:.1f}%)")
        
        # Advanced volumes
        convex_hull_volume = part.get('convex_hull_volume', 0)
        shrinkwrap_volume = part.get('shrinkwrap_volume', 0)
        
        self.project_detail_convex_hull_volume.setText(f"{convex_hull_volume:,.0f} mmT-")
        self.project_detail_shrinkwrap_volume.setText(f"{shrinkwrap_volume:,.0f} mmT-")
        ...
        self.project_detail_sa_vol_ratio.setText(f"{sa_vol_ratio:.3f} mm‘¸¨T¶")
        ...
        self.project_detail_unit_price.setText(f"‘È»{unit_price:.2f}")
        self.project_detail_total_price.setText(f"‘È»{total_price:.2f}")
        
        # Project discount
        project_discount = part.get('project_discount', 0)
        base_unit_price = part.get('base_unit_price', unit_price)
        if project_discount > 0:
            self.project_detail_discount.setText(f"-{project_discount:.1f}% (‘È»{base_unit_price:.2f} ‘ÊÂ ‘È»{unit_price:.2f})")
        elif project_discount < 0:
            self.project_detail_discount.setText(f"+{abs(project_discount):.1f}% (‘È»{base_unit_price:.2f} ‘ÊÂ ‘È»{unit_price:.2f})")
        else:
        ...
        try:
            ...
            if hasattr(self, 'project_stl_viewer') and self.project_stl_viewer:
                ...
                if hasattr(self.project_stl_viewer, 'mesh') and self.project_stl_viewer.mesh:
                    ...
                    self.show_status_message(f"‘ù˘ Optimized {part.get('name', 'part')} - new dimensions: {optimized_features.get('x', 0):.1f} +å {optimized_features.get('y', 0):.1f} +å {optimized_features.get('z', 0):.1f} mm")
                else:
                    self.show_status_message("‘£Ó Optimization failed - could not load mesh")
            else:
                self.show_status_message("‘£Ó 3D viewer not available for optimization")
                
        except Exception as e:
            print(f"Error optimizing bounding box: {e}")
            import traceback
            traceback.print_exc()
            self.show_status_message(f"‘£Ó Optimization failed: {e}")
            ...
            if hasattr(self, 'project_stl_viewer') and self.project_stl_viewer:
                ...
                if hasattr(self.project_stl_viewer, 'mesh') and self.project_stl_viewer.mesh:
                    ...
                    if hasattr(self.project_stl_viewer, 'shrinkwrap') and self.project_stl_viewer.shrinkwrap:
                        ...
                        print(f"Updated shrinkwrap volume: {shrinkwrap_volume:,.0f} mmT-")
                    ...
                    self.show_status_message(f"‘ù˘ Recalculated shrinkwrap for {part.get('name', 'part')} - volume: {part.get('shrinkwrap_volume', 0):,.0f} mmT-")
                else:
                    self.show_status_message("‘£Ó Shrinkwrap calculation failed - could not load mesh")
            else:
                self.show_status_message("‘£Ó 3D viewer not available for shrinkwrap calculation")
                
        except Exception as e:
            print(f"Error recalculating shrinkwrap: {e}")
            import traceback
            traceback.print_exc()
            self.show_status_message(f"‘£Ó Shrinkwrap calculation failed: {e}")
    ...
    def update_project_parts_display(self):
        ...
        for i, part in enumerate(self.project_parts):
            ...
            # Unit price
            unit_price = part.get('unit_price', 0)
            unit_price_item = QtWidgets.QTableWidgetItem(f"‘È»{unit_price:.2f}" if unit_price > 0 else "‘È»-.--")
            self.project_parts_table.setItem(i, 4, unit_price_item)
            
            # Total price
            quantity = part.get('quantity', 1)
            total_price = unit_price * quantity
            total_price_item = QtWidgets.QTableWidgetItem(f"‘È»{total_price:.2f}" if unit_price > 0 else "‘È»-.--")
                ...
                if project_totals:
                    try:
                        ...
                        if project_price and project_price > 0:
                            print(f"   ≠ËÓ- Project-aware price: ‘È»{project_price:.2f} (was ‘È»{unit_price:.2f})")
                        ...
                        print(f"   ‘‹·¥™∆ Project-aware pricing failed: {e}")
                        part['project_discount'] = 0.0
                else:
                    part['project_discount'] = 0.0
    ...
    def predict_project_part_price(self, part, project_totals):
        ...
        try:
            ...
            if hasattr(self, 'feature_names') and hasattr(self, 'project_aware_model') and self.project_aware_model:
                ...
                print(f"   ≠Ëˆè DEBUG - Feature Vector Completeness:")
                ...
                if project_features:
                    print(f"   ≠Ë∆å¥™∆ Project Features in Model:")
            ...
            print(f"   ≠Ëˆè DEBUG - Comparing predictions:")
            ...
            print(f"      Individual price: ‘È»{individual_price:.2f}")
            ...
            if isinstance(project_result, tuple):
                ...
                project_price = project_result
            print(f"      Project-aware price: ‘È»{project_price:.2f}")
    ...
    def update_project_totals_display(self):
        ...
        if not self.project_parts:
            ...
            self.total_volume_label.setText("0.00 mmT-")
            self.total_surface_area_label.setText("0.00 mmT-")
            self.total_bb_volume_label.setText("0.00 mmT-")
            self.total_convex_hull_label.setText("0.00 mmT-")
            self.total_shrinkwrap_label.setText("0.00 mmT-")
            self.total_price_label.setText("‘È»0.00")
            return
        ...
        self.total_volume_label.setText(f"{total_volume:.2f} mmT-")
        self.total_surface_area_label.setText(f"{total_surface_area:.2f} mmT-")
        self.total_bb_volume_label.setText(f"{total_bb_volume:.2f} mmT-")
        self.total_convex_hull_label.setText(f"{total_convex_hull:.2f} mmT-")
        self.total_shrinkwrap_label.setText(f"{total_shrinkwrap:.2f} mmT-")
        
        # Display price with discount info if applicable
        if project_discount_applied and project_discount_percent > 0:
            price_text = f"‘È»{total_price:.2f}"
            if total_base_price != total_price:
                price_text += f" (was ‘È»{total_base_price:.2f}, -{project_discount_percent:.1f}% project discount)"
            self.total_price_label.setText(price_text)
        else:
            self.total_price_label.setText(f"‘È»{total_price:.2f}")
    
    def calculate_all_prices(self):
        """Calculate prices for all parts using enhanced Lasso regression"""
        print("≠Ëƒª Calculate All Prices button clicked!")
        
        # Check if we have parts to process first
        if not hasattr(self, 'project_parts') or not self.project_parts:
            self.show_status_message("‘£Ó No parts in project! Upload STL files first.")
            print("‘£Ó No project parts found")
            return
        
        print(f"‘ù˘ Found {len(self.project_parts)} parts to process")
        
        # Check if enhanced pricing is available, if not try to initialize it
        # Force enhanced pricing initialization every time
        print("≠Ëˆ‰ Force initializing enhanced pricing model...")
        try:
            from enhanced_pricing_integration import initialize_enhanced_pricing, enhanced_calculator
            print("≠Ëˆè Calling initialize_enhanced_pricing()...")
            result = initialize_enhanced_pricing()
            print(f"≠Ëˆè Initialize result: {result}")
            print(f"≠Ëˆè Calculator model loaded: {enhanced_calculator.model is not None}")
            print(f"≠Ëˆè Calculator scaler loaded: {enhanced_calculator.scaler is not None}")
            
            if result and enhanced_calculator.model is not None:
                self.enhanced_pricing_available = True
                print("‘ù˘ Enhanced pricing model initialized successfully!")
                ...
                print(f"≠Ëˆè Test prediction: ‘È»{test_price}")
                
            else:
                print("‘£Ó Enhanced pricing model initialization failed or model is None")
                self.enhanced_pricing_available = False
        except Exception as e:
            print(f"‘£Ó Enhanced pricing model not available: {e}")
            import traceback
            traceback.print_exc()
            self.enhanced_pricing_available = False
        ...
        if not self.enhanced_pricing_available:
            print("≠Ëˆ‰ Falling back to standard pricing model...")
            if not hasattr(self, 'pricing_model') or self.pricing_model is None:
                self.show_status_message("‘£Ó No pricing model available! Initializing...")
                print("‘£Ó No pricing model found, attempting to initialize...")
                self.initialize_ai_pricing_model()
                if not hasattr(self, 'pricing_model') or self.pricing_model is None:
                    self.show_status_message("‘£Ó Failed to initialize pricing model!")
                    return
        ...
        print(f"≠ËÙ’ Project Context Analysis:")
        print(f"   Total Parts: {project_totals.get('Total_Parts_in_Project', 0)}")
        print(f"   Total Volume: {project_totals.get('Project_Total_Volume', 0):,.0f} mmT-")
        print(f"   Total Surface Area: {project_totals.get('Project_Total_surface_area', 0):,.0f} mmT-")
        ...
        for i, part in enumerate(self.project_parts):
            try:
                ...
                print(f"\n≠Ëˆ‰ Processing part {i+1}/{len(self.project_parts)}: {part_name}")
                
                # Try project-aware Random Forest model first
                unit_price = None
                print("   ≠ËÓ- Using project-aware Random Forest model (PRIMARY)...")
                try:
                    ...
                    if project_totals:
                        ...
                        if rf_price and rf_price > 0:
                            ...
                            print(f"   ≠ËÓ- Project-aware Random Forest: ‘È»{unit_price:.2f}")
                    ...
                    if unit_price is None:
                        ...
                        if rf_price and rf_price > 0:
                            unit_price = rf_price
                            part['project_discount'] = 0.0
                            print(f"   ≠ËÓ- Standard Random Forest: ‘È»{unit_price:.2f}")
                            
                    if unit_price is None:
                        print("   ‘‹·¥™∆ Random Forest returned invalid price")
                except Exception as e:
                    print(f"   ‘£Ó Random Forest failed: {e}")
                    import traceback
                    traceback.print_exc()
                
                # Fallback to enhanced model if Random Forest failed
                if unit_price is None and self.enhanced_pricing_available:
                    print("   ≠Ëˆ‰ Using Enhanced Lasso model fallback...")
                    try:
                        ...
                        print(f"   ≠Ëˆè Debug: price={debug_price}, error={debug_error}")
                        
                        enhanced_result = calculate_enhanced_price(part)
                        print(f"   ≠Ëˆè Enhanced result type: {type(enhanced_result)}")
                        print(f"   ≠Ëˆè Enhanced result: {enhanced_result}")
                        ...
                        if enhanced_price is not None and enhanced_price > 0:
                            unit_price = enhanced_price
                            part['project_discount'] = 0.0
                            print(f"   ≠Ëƒª Enhanced fallback: ‘È»{unit_price:.2f}")
                            ...
                            print(f"   ‘‹·¥™∆ Enhanced model returned None")
                    except Exception as e:
                        print(f"   ‘£Ó Enhanced model failed: {e}")
                        import traceback
                        traceback.print_exc()
                ...
                if unit_price is None and hasattr(self, 'pricing_model') and self.pricing_model is not None:
                    print("   ≠Ëˆ‰ Using standard linear model fallback...")
                    try:
                        ...
                        part['project_discount'] = 0.0
                        print(f"   ≠ËÙ’ Standard linear model: ‘È»{unit_price:.2f}")
                    except Exception as e:
                        print(f"   ‘£Ó Standard linear model failed: {e}")
                        unit_price = 10.0  # Default fallback
                
                # Final fallback
                if unit_price is None:
                    unit_price = 10.0
                    print(f"   ‘‹·¥™∆ Using default price: ‘È»{unit_price:.2f}")
                
                quantity = part.get('quantity', 1)
                total_price = unit_price * quantity
                total_project_price += total_price
                
                print(f"   ‘ù˘ Final: Unit=‘È»{unit_price:.2f}, Qty={quantity}, Total=‘È»{total_price:.2f}")
                
                # Store unit price in part data for later use
                part['unit_price'] = unit_price
                
            except Exception as e:
                print(f"‘£Ó Error processing {part.get('name', 'Unknown')}: {e}")
                import traceback
                traceback.print_exc()
        ...
        print(f"\n≠Ëƒª FINAL PROJECT TOTAL: ‘È»{total_project_price:.2f}")
        
        model_type = "Project-aware Random Forest" if project_totals else "Standard Random Forest"
        self.show_status_message(f"‘ù˘ Calculated prices using {model_type} model for {len(self.project_parts)} parts with full project context!")
    ...
    def show_part_features(self, part):
        ...
        try:
            if hasattr(self, 'enhanced_pricing_available') and self.enhanced_pricing_available:
                try:
                    ...
                    if enhanced_breakdown:
                        ...
                        html_content = []
                        html_content.append(f"<h3>≠Ë‹« Enhanced Feature Analysis</h3>")
                        ...
                        html_content.append(f"<b>Predicted Price:</b> ‘È»{enhanced_breakdown.get('predicted_price', 0):.2f}<br>")
                        ...
                        if 'top_features' in enhanced_breakdown:
                            html_content.append("<h4>≠Ëˆπ Top Contributing Features:</h4>")
                            for i, (feature, contribution) in enumerate(enhanced_breakdown['top_features'][:10], 1):
                                direction = "‘Êå¥™∆" if contribution > 0 else "‘Êú¥™∆"
                                html_content.append(f"{i}. <b>{feature}</b> {direction} ‘È»{contribution:.2f}<br>")
                        
                        # Feature categories
                        if 'feature_categories' in enhanced_breakdown:
                            html_content.append("<br><h4>≠ËÙ’ Feature Categories:</h4>")
                            for category, impact in enhanced_breakdown['feature_categories'].items():
                                html_content.append(f"‘«Û <b>{category}:</b> ‘È»{impact:.2f}<br>")
                        
                        # Base features
                        html_content.append("<br><h4>≠ËÙ∆ Base Part Features:</h4>")
                        base_features = [
                            ('Volume', part.get('volume', 0), 'mmT-'),
                            ('Surface Area', part.get('surface_area', 0), 'mmT-'),
                            ('Convex Hull Volume', part.get('convex_hull_volume', 0), 'mmT-'),
                            ('Shrinkwrap Volume', part.get('shrinkwrap_volume', 0), 'mmT-'),
                        ...
                        for name, value, unit in base_features:
                            html_content.append(f"‘«Û <b>{name}:</b> {value:.2f} {unit}<br>")
            ...
            if hasattr(self, 'pricing_model') and self.pricing_model is not None:
                ...
                html_content = []
                html_content.append(f"<h3>≠ËÙ’ Standard Feature Analysis</h3>")
                html_content.append(f"<b>Part:</b> {part.get('name', 'Unknown')}<br>")
                html_content.append(f"<b>Model:</b> Standard Lasso Regression<br><br>")
                
                # Feature coefficients
                if hasattr(self, 'feature_coefficients'):
                    html_content.append("<h4>≠Ëˆè Feature Coefficients:</h4>")
                    for feature, coef in self.feature_coefficients.items():
                        if coef != 0:
                            direction = "‘Êå¥™∆" if coef > 0 else "‘Êú¥™∆"
                            html_content.append(f"‘«Û <b>{feature}:</b> {coef:.3f} {direction}<br>")
                
                # Base features
                html_content.append("<br><h4>≠ËÙ∆ Part Features:</h4>")
                base_features = [
                    ('Volume', part.get('volume', 0), 'mmT-'),
                    ('Surface Area', part.get('surface_area', 0), 'mmT-'),
                    ('Convex Hull Volume', part.get('convex_hull_volume', 0), 'mmT-'),
    ...
    def update_shrinkwrap(self):
        ...
        if hasattr(self, 'stl_viewer') and self.stl_viewer.current_file:
            ...
            self.show_status_message("≠Ëˆ‰ Shrinkwrap updated", 2000)
        else:
            self.show_status_message("‘‹·¥™∆ No file loaded", 2000)
    ...
    def on_pricing_model_changed(self, model_name):
        ...
        if model_name and model_name in self.available_models:
            ...
            self.show_status_message(f"≠ËÙ’ Selected model: {model_name}", 2000)
        else:
            self.show_status_message("‘‹·¥™∆ Invalid model selection", 2000)
    ...
    def convert_step_to_stl(self):
        ...
        self.step_conversion_thread = StepConversionThread(step_file, stl_file, resolution)
    ...
    def show_pricing_analysis(self):
        ...
        try:
            ...
            if not os.path.exists(report_path):
                ...
                basic_report = """# ≠Ëƒª 3D PRINTING PRICING ANALYSIS REPORT
============================================================

≠ËÙ’ **Dataset Overview:**
   ‘«Û Total parts analyzed: 2,275
   ‘«Û Unique projects: 695
   ‘«Û Price range: ‘È»0.10 - ‘È»3646.21
   ‘«Û Average price per part: ‘È»66.44
   ‘«Û Average price per mmT-: ‘È»0.001560

≠Ëˆå **Volume-Price Relationship:**
   ‘«Û Correlation coefficient: 0.776
   ‘«Û Strong positive correlation - larger parts cost significantly more

≠ËÙé **Quantity Discounts:**
   ‘«Û Single part: ‘È»0.001398/mmT-
   ‘«Û Bulk (‘Îπ10): ‘È»0.001374/mmT-
   ‘«Û Bulk discount: 1.7%

≠Ëˆû **Complexity Premium:**
   ‘«Û Simple parts (low surface/volume): ‘È»0.000201/mmT-
   ‘«Û Complex parts (high surface/volume): ‘È»0.002299/mmT-
   ‘«Û Complexity premium: 1042.3%

≠Ë∆å¥™∆ **Project Size Effects:**
   ‘«Û Small projects (‘Î•10 parts): ‘È»0.001444/mmT-
   ‘«Û Large projects (‘Îπ100 parts): ‘È»0.000985/mmT-
   ‘«Û Large project discount: 31.8%

## ≠Ë•æ MACHINE LEARNING INSIGHTS
----------------------------------------
**Model Performance Comparison:**
   ‘«Û Random Forest: RT- = 0.920, MAE = ‘È»10.57
   ‘«Û Gradient Boosting: RT- = 0.912, MAE = ‘È»11.81
   ‘«Û Linear Regression: RT- = 0.866, MAE = ‘È»20.88

**Most Important Pricing Factors (Random Forest):**
   ‘«Û convex_hull_volume: 0.253
   ‘«Û shrinkwrap_volume: 0.249
   ‘«Û surface_area: 0.249
   ‘«Û bb_volume: 0.221
   ‘«Û Volume: 0.012

## ≠Ëƒª KEY FINDINGS & RECOMMENDATIONS
----------------------------------------
...
**Optimization Strategies:**
   ‘«Û Consolidate small parts into larger orders for quantity discounts
   ‘«Û Group multiple projects together to reach large project thresholds
   ‘«Û Optimize part geometry to reduce surface area while maintaining function
   ‘«Û Consider material efficiency - minimize bounding box waste
"""
            ...
            tab_widget.addTab(dashboard_tab, "≠ËÙ’ Dashboard")
            tab_widget.addTab(scatter_tab, "≠ËÙ≥ Correlations")
            tab_widget.addTab(examples_tab, "≠Ë∆å¥™∆ Project Examples")
            tab_widget.addTab(insights_tab, "≠Ëƒª Key Insights")
            tab_widget.addTab(recommendations_tab, "≠ËÂÌ Recommendations")
            tab_widget.addTab(raw_tab, "≠ËÙ‰ Raw Data")
    ...
    def create_project_examples(self, report_content):
        ...
        content = """
# ≠Ë∆∫ Real Project Examples & Deep Analysis

### Large Quantity vs Single Part Comparison

#### **High Volume Project: P-00625 (senzor_print_5)**
- **Quantity**: 2,000 parts
- **Price per part**: ‘È»2.98  
- **Price per mmT-**: ‘È»0.000386
- **Volume**: 7,718 mmT-
- **Analysis**: Excellent economies of scale - bulk production drives cost down to ‘È»0.0004/mmT-

#### **Single Part Project: P-00635 (INCOM Stavbe merilo 50x50)**
- **Quantity**: 1 part
- **Price per part**: ‘È»638.69
- **Price per mmT-**: ‘È»0.000148  
- **Volume**: 4,324,765 mmT-
- **Analysis**: Despite massive volume, single-part setup costs create higher per-mmT- price
...
- Price: ‘È»0.92, Volume: 1,414 mmT- 
- Low surface/volume, minimal waste, compact shape
- **Result**: ‘È»0.00065/mmT- - very efficient

**Complex Part**: P-00580 stikalo-zascita-02 (High Complexity)
- Price: ‘È»5.03, Volume: 124 mmT-
- High surface/volume (4.12), extreme aspect ratio (27.5)
- **Result**: ‘È»0.0404/mmT- - 60x more expensive per volume!

### Why Similar Parts Cost Different

**Case Study**: Perno parts in P-00602
- **3D-3006 Perno**: ‘È»3.20, Complexity: Medium
- **3D-3024 Perno**: ‘È»3.20, Complexity: High
- Same price but different complexity ‘ÊÂ Volume compensation
- Higher complexity part has 50% larger volume, balancing cost

### Bulk Pricing Reality Check
- **Single part average**: ‘È»0.00139/mmT-
- **Bulk (‘Îπ10) average**: ‘È»0.00137/mmT-  
- **Actual bulk discount**: 1.7%
- **Large projects (‘Îπ100)**: ‘È»0.00089/mmT- (36% discount)

**Real Examples from Dataset:**
‘«Û **Single part**: P-00573 (‘È»193.89) vs P-00591 bulk (‘È»0.92)
‘«Û **Bulk (‘Îπ10)**: P-00557 (8 parts, ‘È»18.13 each) vs similar singles
‘«Û **Bulk discount**: P-00625 shows 1.7% savings pattern
...
**Conclusion**: Complexity matters more than volume for pricing!
    ...
    def extract_key_insights(self, report_content):
        ...
        content = """
# ≠Ëˆè Key Pricing Intelligence

## ≠ËÙ’ Dataset Overview
- Analyzed 2,275 parts across multiple industries
- Data from 695 unique customer projects  
- Average part cost: ‘È»66.44
- Price spectrum: ‘È»0.10 - ‘È»3646.21

## ≠ËÙ≥ Volume-Price Correlation
...
## ≠ËÂ- Discount Opportunities
- **Bulk orders (‘Îπ10 parts):** Save 1.7% on per-volume pricing
- **Large projects (‘Îπ100 parts):** Save up to 31.8% - massive discount potential!
- Project consolidation strategy can reduce costs significantly
- Single parts pay premium rates - batch whenever possible

## ≠Ëˆû Complexity Cost Impact
- **Simple parts:** ‘È»0.000201/mmT- - most cost-effective
- **Complex parts:** ‘È»0.002299/mmT- - over 10x more expensive!
- **Complexity premium:** 1042.3% increase for high surface-to-volume ratios
- Design optimization can dramatically reduce costs

## ≠Ë•æ AI Model Performance
- **Random Forest Model:** RT- = 0.920, MAE = ‘È»10.57 (Best performer)
...
- **Reliability:** Average prediction error only ‘È»10.57 per part

## ≠Ëƒª Strategic Insights
...
- Consolidation strategies offer the biggest savings potential
    ...
    def extract_recommendations(self, report_content):
        ...
        content = """
# ≠ËÂÌ Strategic Cost Optimization Guide

## ≠Ëƒª Primary Cost Reduction Strategies

### ≠ËÙé Volume Consolidation Strategy
Combine orders to reach bulk thresholds. Even a 1.7% discount on large orders can save hundreds of euros on major projects.

### ≠Ë∆å¥™∆ Project Scaling Approach
...
### ≠Ëˆû Design Complexity Optimization
Reduce surface-to-volume ratios where possible. Complex parts cost over 10x more per mmT- than simple designs.

### ≠ËÙ’ Data-Driven Pricing
...
## ≠ËÂ- Real Savings Examples

**Example 1:** Instead of ordering 50 parts individually (‘È»0.001398/mmT-), batch them together to save 1.7% = ‘È»23.50 on a ‘È»1,400 order

**Example 2:** Combine 3 small projects (30 parts each) into one large project (90 parts) to approach the 100+ part threshold for 31.8% discount = ‘È»890 savings on a ‘È»2,800 order

**Example 3:** Redesign complex part (‘È»0.002299/mmT-) to simple geometry (‘È»0.000201/mmT-) = 91% cost reduction on that part

## ‘‹Ì Advanced Optimization Techniques

### ≠Ëˆè Geometric Analysis
...
- Optimize geometric algorithms
    ...
    def create_scatter_plot_tab(self):
        ...
        # Title
        title_label = QtWidgets.QLabel("≠ËÙ≥ Geometric Features vs Price Correlations")
        ...
        def update_plot():
            ...
            try:
                ...
                ax.set_ylabel('Price (‘È»)', fontsize=12, fontweight='bold')
                        ...
                        if log_checkbox.isChecked():
                            ...
                            if np.sum(valid_idx) > actual_degree:
                                ...
                                ax.plot(x_trend, y_trend, color=color, linestyle=style, 
                                       ...
                                       label=f'{actual_degree}T- Polynomial Fit')
                        else:
...
class PricingPresetsDialog(QtWidgets.QDialog):
    ...
    def create_machine_settings_group(self, preset):
        ...
        machine_investment.setPrefix("‘È»")
        ...
        annual_maintenance.setPrefix("‘È»")
    ...
    def create_labor_settings_group(self, preset):
        ...
        monitoring_time.setSuffix(" +å build time")
    ...
    def create_pricing_settings_group(self, preset):
        ...
        maintenance_buffer.setPrefix("‘È»")
