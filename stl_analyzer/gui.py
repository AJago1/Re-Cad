...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setup_3d_viewer(self):
        """Set up the 3D viewer panel using the specialized widget"""
        # Import and create the 3D viewer widget
        try:
            from .gui.widgets import STLViewerWidget
        except ImportError:
            try:
                from gui.widgets import STLViewerWidget
            except ImportError:
                from stl_analyzer.gui.widgets import STLViewerWidget
        
        # Create and return the 3D viewer widget
        return STLViewerWidget(parent=self)
    ...
    def setup_price_calculator_page(self):
        ...
        # Create 3D viewer for price calculator (needed for compatibility)
        try:
            from viewer import STLViewer
            self.price_calc_viewer = STLViewer(self)
        except ImportError:
            self.price_calc_viewer = None
            print("Warning: STLViewer not available for price calculator")
    ...
    def setup_project_pricing_page_implementation(self):
        """Implementation of the project pricing page UI - called by ProjectPricingTab"""
    ...
    def on_part_selection_changed(self, current, previous):
        ...
        if current:
            ...
            if file_path and file_path in self.price_calc_parts:
                ...
                # Update the 3D viewer (with error handling)
                if os.path.exists(file_path) and hasattr(self, 'price_calc_viewer') and self.price_calc_viewer is not None:
                    try:
                        self.price_calc_viewer.load_stl(file_path)
                    except Exception as e:
                        print(f"Error loading file in price calc viewer: {e}")
                ...
                if hasattr(self, 'price_calc_viewer') and self.price_calc_viewer is not None:
                    try:
                        self.price_calc_viewer.clear()
                    except Exception as e:
                        print(f"Error clearing price calc viewer: {e}")
        else:
    ...
    def on_project_part_clicked(self, item):
        ...
        if hasattr(self, 'project_stl_viewer') and self.project_stl_viewer is not None and 'file_path' in part:
            try:
...
class PricingPresetsDialog(QtWidgets.QDialog):
    ...
    def update_shrinkwrap(self):
        """Update shrinkwrap for the current model"""
        if hasattr(self, 'stl_viewer') and self.stl_viewer.stl_path:
            try:
                self.stl_viewer.update_shrinkwrap()
                self.show_status_message("🔄 Shrinkwrap updated", 2000)
            except Exception as e:
                self.show_status_message(f"⚠️ Shrinkwrap update failed: {str(e)}", 3000)
        else:
            self.show_status_message("⚠️ No file loaded", 2000)
    ...
    def on_optimization_complete(self, file_path, optimized_features):
        """Handle optimization completion from 3D viewer"""
        try:
            if optimized_features:
                # Update the database with optimized features
                self.stl_db.update_entry(file_path, optimized_features)
                self.stl_db.save_database()
                ...
                self.show_status_message(f"✅ Optimization complete: {os.path.basename(file_path)}", 3000)
            else:
                self.show_status_message("⚠️ Optimization failed", 3000)
        except Exception as e:
            self.show_status_message(f"⚠️ Error updating optimization: {str(e)}", 3000)
